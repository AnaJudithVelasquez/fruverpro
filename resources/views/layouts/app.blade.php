...
<body class="bg-green-50 font-sans antialiased min-h-screen flex flex-col">
    ...
    <nav class="bg-green-600 text-white p-4 flex justify-between items-center shadow-md">
        ...
        <div class="flex items-center gap-4">
            ...
            <a href="{{ route('productos.index') }}" class="hover:bg-green-700 px-3 py-2 rounded-md transition">
                Productos
            </a>

            <a href="{{ route('compras.index') }}" class="hover:bg-green-700 px-3 py-2 rounded-md transition">
                Compras
            </a>

            <a href="{{ route('ventas.index') }}" class="hover:bg-green-700 px-3 py-2 rounded-md transition">
                Ventas
            </a>

            <a href="{{ route('profile.edit') }}" class="hover:bg-green-700 px-3 py-2 rounded-md transition">
                Perfil
            </a>

            <form method="POST" action="{{ route('logout') }}" class="inline">
                @csrf
                <button type="submit" class="hover:bg-green-700 px-3 py-2 rounded-md transition">
                    Cerrar sesión
                </button>
            </form>
        </div>
